...
async function init() {
    try {
        ...
        if (data) {
            renderAll();
            // Cross-benchmark aggregate: independent of the benchmark
            // selector and framing toggles, so rendered once here.
 ...
 */
/* ============================================================
 * Threshold-crossing gaps — cross-benchmark aggregate (Plotly)
 * Data: data.threshold_aggregate (see scripts/threshold_gap.py)
 * ============================================================ */

const THRESHOLD_TREND_COLORS = {
    public: '#1f4e8e',
    private: '#9c1f1f',
    overall: '#374151',
};
const THRESHOLD_TREND_FILLS = {
    public: 'rgba(31, 78, 142, 0.12)',
    private: 'rgba(156, 31, 31, 0.12)',
    overall: 'rgba(55, 65, 81, 0.12)',
};
const THRESHOLD_BENCH_PALETTE = [
    '#0ea5e9', '#f59e0b', '#10b981', '#8b5cf6',
    '#ec4899', '#14b8a6', '#f97316', '#64748b',
];

function fmtThresholdValue(t) {
    return Math.abs(t) < 2 ? t.toFixed(2) : String(Math.round(t));
}
...
function renderThresholdChart(data) {
    const agg = data?.threshold_aggregate;
    const section = document.getElementById('threshold-section');
    if (!section) return;
    if (!agg || !Array.isArray(agg.datapoints) || agg.datapoints.length === 0) {
        // data.json predates the threshold analysis — keep the section hidden.
        return;
    }
    ...
    // Trend lines with 90% bootstrap CI bands. Prefer the public/private
    // split (the source analysis's headline comparison); fall back to the
    // overall trend when the split is unavailable.
    const trendKeys = ['public', 'private'].filter(k => agg.trends?.[k]);
    const useKeys = trendKeys.length > 0 ? trendKeys : (agg.trends?.overall ? ['overall'] : []);
    for (const key of useKeys) {
        const trend = agg.trends[key];
        const band = trend.points.filter(p => p.lo !== null && p.hi !== null);
        if (band.length > 1) {
            traces.push({
                x: band.map(p => p.date),
                y: band.map(p => p.hi),
            ...
            traces.push({
                ...
                line: { width: 0 },
                fill: 'tonexty',
                fillcolor: THRESHOLD_TREND_FILLS[key],
                hoverinfo: 'skip',
                showlegend: false,
            });
        }
        const label = key === 'overall'
            ? `Overall trend (${trend.n_benchmarks} benchmarks)`
            : `${key.charAt(0).toUpperCase() + key.slice(1)} data (${trend.n_benchmarks} benchmarks)`;
        traces.push({
            x: trend.points.map(p => p.date),
            y: trend.points.map(p => p.mean),
            mode: 'lines',
            name: label,
            line: { color: THRESHOLD_TREND_COLORS[key], width: 2.8 },
            hovertemplate: `${label}: %{y:.1f} months<extra></extra>`,
        });
    }

    // Scatter: one trace per benchmark; marker shape encodes data access
    // (circle = public test set, star = private/contamination-resistant).
    const benchIds = [...new Set(agg.datapoints.map(p => p.benchmark_id))].sort();
    const benchColor = {};
    benchIds.forEach((id, i) => {
        benchColor[id] = THRESHOLD_BENCH_PALETTE[i % THRESHOLD_BENCH_PALETTE.length];
    });
    for (const id of benchIds) {
        const pts = agg.datapoints.filter(p => p.benchmark_id === id);
        const isPrivate = pts[0].data_access === 'private';
        traces.push({
            x: pts.map(p => p.first_open_date),
            y: pts.map(p => p.gap_months),
            mode: 'markers',
            name: pts[0].benchmark_name + (isPrivate ? ' ★' : ''),
            marker: {
                symbol: isPrivate ? 'star' : 'circle',
                size: isPrivate ? 13 : 9,
                color: benchColor[id],
            ...
            customdata: pts.map(p => [
                p.benchmark_name,
                fmtThresholdValue(p.threshold),
                p.first_closed_model,
                fmtMonthYear(p.first_closed_date),
                p.first_open_model,
                fmtMonthYear(p.first_open_date),
            ]),
            hovertemplate:
                '<b>%{customdata[0]}</b> · threshold %{customdata[1]}<br>' +
                'Closed first: %{customdata[2]} (%{customdata[3]})<br>' +
                'Open first: %{customdata[4]} (%{customdata[5]})<br>' +
                'Gap: %{y:.1f} months<extra></extra>',
        });
    }
    ...
    const layout = {
        ...
        xaxis: {
            title: 'Date the open model reached the threshold',
            titlefont: { size: 12, color: COLORS.annotation },
            tickfont: { size: 11, color: COLORS.annotation },
            gridcolor: COLORS.gridline,
            zeroline: false,
        ...
        yaxis: {
            title: 'Months behind the closed frontier',
            titlefont: { size: 12, color: COLORS.annotation },
            tickfont: { size: 11, color: COLORS.annotation },
            gridcolor: COLORS.gridline,
            zeroline: false,
        ...
        shapes: [{
            ...
            line: { color: COLORS.annotation, width: 0.5 },
        }],
        legend: {
            orientation: 'h',
            yanchor: 'top',
    ...
    Plotly.newPlot('threshold-chart', traces, layout, config);

    // Medians caption
    const note = document.getElementById('threshold-note');
    if (note && agg.medians) {
        const parts = [];
        if (agg.medians.overall !== undefined) parts.push(`overall <strong>${agg.medians.overall} mo</strong>`);
        if (agg.medians.public !== undefined) parts.push(`public test sets <strong>${agg.medians.public} mo</strong>`);
        if (agg.medians.private !== undefined) parts.push(`private test sets <strong>${agg.medians.private} mo</strong>`);
        const nBench = new Set(agg.datapoints.map(p => p.benchmark_id)).size;
        note.innerHTML = `Median gap: ${parts.join(' · ')} — ${agg.datapoints.length} datapoints across ${nBench} benchmarks.`;
    }
}
